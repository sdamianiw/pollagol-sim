...
> argmax 0-1 ≠ modal 1-1) + Sebas-mandated deep council (champion double-exposure vs P17's Portugal).

## MANDATE
Confirm or challenge the pre-entered **España 1-2**; adjudicate the placement/hedge question ("fire
placement-MC or stay chalk?") given our España champion + P17 (2nd, −17, Portugal champion).
...
- **G5 hedge portfolio (binary criterion, fixed before running):** branch probs at 120′: ESP-win 0.575 /
  ...
  Expected gap-change vs P17 tonight (match + champion-liveness swing, q = P(title|QF) ∈ {.15,.25,.35};
  market lens: q ≈ 0.19 BOTH teams):
  chalk **1-2: +1.68…+2.27** · 0-1: +1.63…+2.22 · **hedge POR 2-1: +0.44…+1.03** (P17-enters-POR case);
  in the P17-chalk case hedge goes NEGATIVE (−0.79…−0.20). **Hedge dominated everywhere in the band →
...
- **Adversarial/premortem:** FM1 (POR wins → P17 overtakes: −3 net after champion swing) = LANDS-BUT-
...
- **Rival/field:** Spain-advance = best branch on BOTH layers (kills P17's champion+GK, neutral vs
  P02); Portugal-advance = common-mode España kill WITH P02 (he also loses Yamal MVP → actually widens
  our P02 gap) — the two-front problem resolves in favor of chalk; P17 PRIOR 80-90% enters a POR win.
...
2. **Hedge/placement question: STAY CHALK — binary criterion decisive** (chalk beats hedge by ~+1.2
   expected gap-points vs P17 in EVERY q × P17-entry scenario; hedge also bleeds vs P02 and field).
...
Worst-case on record (FM1): POR advances (35.2%) → our España dies, P17's lives; gap math says we then
...
| C — Hedge criterion construction | **SOUND** — the whole-band requirement (q up to 0.35, market-implied ≈0.19) is the conservative direction; the P17-entry prior (80-90% POR) is unverified but the conclusion is robust to it being wrong (hedge goes negative in the P17-chalk case). No circularity. |
