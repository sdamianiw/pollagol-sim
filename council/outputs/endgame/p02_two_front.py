...
Cross-validation (L62 rule): the pairwise marginal P(P02 passes us | pure strategy) from THIS sim must
  match p02_endgame.py's EXACT convolution within 3xSE.
Usage: python council/outputs/endgame/p02_two_front.py [snapshot.json]
"""
import sys, os
...
import p02_endgame as GE
from p02_endgame_mc import np_score, sample_cells

SEED = 42
N = 1_000_000
POINTS = {"us": 386, "P02": 362, "P11": 342, "P05": 342, "P17": 339, "rodrigo": 327}
...
STRATEGIES = [                                 # the P02 decision variable
...
def run(quiet=False, p02_wins_chaser_ties=False, contrarian_tilt=0.0, p_olise_shift=0.0):
    """One full joint sim. Returns {strategy: (P1, P02, P3, Ppass_us, Eprize)} for P02.
    p02_wins_chaser_ties: tie convention vs chasers (unknown pleno counts) -- run both.
    ...
    chasers = {}
    for name in ("P11", "P05", "P17", "rodrigo"):
    ...
    champ_pay = {"us": esp.astype(float) * LOCK, "P02": esp.astype(float) * LOCK}
    ...
    awards = {
        ...
        "P02": champ_pay["P02"] + LOCK * (boot_mba + mvp_yam + gk_mai + ast_messi),
        "P11": LOCK * (boot_mba + mvp_yam + ast_olise),
        "P05": LOCK * (boot_kane + mvp_yam + gk_dibu),
        "P17": LOCK * (boot_mba + mvp_mba),
    ...
    finals = {n: POINTS[n] + chasers[n][0] + chasers[n][1] + awards[n]
              for n in ("P11", "P05", "P17", "rodrigo")}
    finals["us"] = POINTS["us"] + us_match + awards["us"]

    out = {}
    for label, g1, g2 in STRATEGIES:
        p02_match = np_score(np.float64(g1[0]), np.float64(g1[1]), c3h, c3a) \
            + np_score(np.float64(g2[0]), np.float64(g2[1]), cfh, cfa)
        p02_total = POINTS["P02"] + p02_match + awards["P02"]
        # rank vs us: we hold the pleno tiebreak (18 vs ~15) -> tie goes to US always.
        beats_us = (p02_total > finals["us"]).astype(float)
        # rank vs chasers: pleno counts unknown -> convention parameter.
        if p02_wins_chaser_ties:
            above = sum((finals[n] > p02_total).astype(float) for n in ("P11", "P05", "P17", "rodrigo"))
        else:
            above = sum((finals[n] >= p02_total).astype(float) for n in ("P11", "P05", "P17", "rodrigo"))
        rank = 1.0 + (1.0 - beats_us) + above                            # 1 + players strictly above P02
        ...
        if not quiet:
            print(f"  {label:24s} P1={p1*100:6.3f}%  P02={p2*100:6.2f}%  P3={p3*100:6.2f}%  "
                  f"E[prize-share]={ep:.4f}  (passes us: {float(beats_us.mean())*100:5.3f}%)")
    return out


def main():
    print(f"P02 TWO-FRONT (MC N={N:,} seed={SEED})  boards: P02 362 | chasers 342/342/339/327 | us 386")
    print(f"  chaser mixes ASSUMED chalk-heavy; awards: boot {P_BOOT} mvp {P_MVP} gk {P_GK} ast {P_AST}")
    print("\n=== [1] P02 strategy table (chaser ties count AGAINST P02 -- conservative for his seed-2) ===")
    base = run()

    print("\n=== [2] tie convention flipped (P02 WINS chaser ties) ===")
    run(p02_wins_chaser_ties=True)
    ...
    # ---- L62 cross-validation: pairwise P(P02 passes us) vs the EXACT convolution ----
    print("\n=== [5] CROSS-VAL: pairwise P(passes us) vs p02_endgame EXACT (pure entries, 3xSE gate) ===")
    cells3, cellsF = GE.dist_cells(GE.FID3), GE.dist_cells(GE.FIDF)
    ok_all = True
