...
binds) · harness `p02_endgame.py` EXACT convolution, MC cross-validated 5/5 within 3×SE (L62 rule),
...
- **FINAL Jul-19: ENTER SPAIN 1-0** — engine FULL120 argmax (E 2.728) AND P(hold)-argmax for every
  realistic read of P02. **The cover (0-1 ARG) does NOT fire at +24** — see the q-analysis below.
  Unanimous 4/4 lenses + the P02 role-play's own math.
...
## Harness decision tables (exact; full run `p02_endgame_run_2026-07-17.txt`)
- **[A] FINAL at gap +24, P(pass+tie) by q = P(P02 ARG-side):** chalk 1-0 best for q ≤ **0.81**
...
- **[D] adversarial (P02 pure best-response):** worst-case vs (2-1, 1-0) = 0.501%; minimax (1-0, 1-0)
  = 0.465%. **3.6bp of insurance for ~9bp behavioral cost (2.5:1 against) → stay behavioral-best 2-1.**
  P02's computed best responses: g1 = 1-2 ENG, g2 = **2-1 ESP** (see role-play convergence below).
...
## THE READ — "will P02 stay Spain or flip to Argentina?" (the user's tight call)
...
1. **q-insensitivity (the load-bearing result):** under the strict-pass metric the cover buys ≤ 2–3bp
   at ANY q ∈ [0,1] under EVERY stressed award scenario. There is no value of the P02-read that flips
...
2. **Rational-P02 goes SPAIN-side (role-play, independent):** an ARG title triggers our Dibu (+10 us),
...
rational-P02's implied fear (a leader who enters the modal Spain score blocks his exact-2-1 kill-shot
...
## T-1h ENTER rules (numeric; re-run `p02_endgame.py <fresh-snap>` + `ko` at each node)
...
- Default **SPAIN 1-0**. p_over ≥ .50 → 2-1 ESP competes (re-run ko). ARG devig ≥ .33 (engine winner-flip
  boundary) → re-derive; ARG ≥ .38 + credible P02-ARG signal → re-run cover harness with fresh numbers.
...
## What must happen for P02 (kill-shot map, for the record)
...
2-2/0-0 — every worst case and P02 best-response byte-identical); `joint()` gap parameterized (removes
...
economically near-irrelevant; the P02 fortress (~99%) dominates everything.
**RECONCILIATION vs the main verdict (does chalk-chalk hold? YES — strengthened):**
1. The E[prize]-rational P02 tilts to the DOUBLE differential (ENG 1-2 + **ARG 0-1** — the two-front
   ...
   already priced**, and our chalk 1-0 remains the argmin vs ARG-side P02 (that's the q-insensitivity
   result: cover ≤2-3bp strict at any q). Correction of an earlier framing (V1 audit): rational P02
...
2. Sebas's headline question answered: **the England gamble ALONE is P02's worst move** (−6bp E,
   P3 ×2.6, buys 0.10pp vs us). If we SEE an England-side P02 signal it implies the double-gamble
...
Note the residual model-spread on P02's rational FINAL entry: P(#1)-maximizer → Spain 2-1 · E[prize]
...
- **3rd place recorded: France 4-6 England** (90'+stoppage, x5 sources). Everyone on the France side scored 0 (us, P02, the field except one non-exact England pick) → **board unchanged: us 386 / P02 362, realized gap +24**, n=103, plenos 18.
...
- **`final_node.py`** (thin driver reusing p02_endgame fns; game-1 resolved → final-only at gap +24; B4 1-hash): 1-0 ESP = **EV-argmax AND P(hold)-argmin for all q≤.75**; the 0-1 cover is **negative-value in every stressed award sweep** (−14 to −54bp) and buys only 3-5bp even at q=.90-1.0. **Cover gate TRIPLE-SHUT**: gap 24>20 · no hard-q evidence · **Olise added 1 assist (7 total, breaks Pelé's record)** — P02's Messi-AST leg needs Messi 3+ assists tonight to TIE → re-grounded MESSI {ESP .03/ARG .08} ⇒ **P(hold strict) = 99.976%** on 1-0 ESP.
- Intel: Yamal + Porro both trained apart Thu but **expected to play** (ESPN) — no ko rerun trigger. Ball: Messi −900 (common-null). Boot: Mbappé 10 vs Messi 8 (needs 3+) — our locked Mbappé-Boot leg near-certain, common-mode vs P02.
