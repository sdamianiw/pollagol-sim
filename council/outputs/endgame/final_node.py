...
both we and P02 scored 0 on it (board-confirmed), so the realized gap is +24 and the decision
collapses to the final-only lookup the p02_endgame harness pre-computed as section [E].
This driver reuses p02_endgame.py functions verbatim (no logic re-implementation) on the FRESH
...
def table(cellsF, qs, gap):
    print(f"\n[A] FINAL-ONLY e2 table @ realized gap +{gap}  (P(P02 passes)+P(tie), strict: tie=loss)")
