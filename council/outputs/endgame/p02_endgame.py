# -*- coding: ascii -*-
"""P02 ENDGAME -- exact enumeration of the last two nodes (3rd-place Jul-18 + FINAL Jul-19), 2026-07-17.
ADVISORY; I-3 clean (reads snapshot odds only, imported by nothing). EXACT convolution, no Monte Carlo.

Model (successor of eng_arg_sf/p02_paths.py, which stays frozen as the SF2 audit artifact):
  gap = +24 (board 2026-07-17: us 386 / P02 362). Two scored games remain.
  Game1 3rd-place France-England: PURE match layer vs P02 (his award legs don't touch it).
  Game2 FINAL Spain-Argentina: THE champion node -- result region sets champ in {ESP, ARG} directly
    (level after 120' -> pens, champ ~ PENS_ESP). Champion + Boot legs are COMMON-MODE with P02
    (both hold Espana + Mbappe) -> delta 0 always, they drop out of the gap arithmetic entirely.
  Live differential award legs (delta = P02-minus-us, LOCK=10):
    P02: Messi-ASSISTER (+10, champ-conditioned), Yamal-MVP (+10), Maignan-GK (+10, ~dead)
  ...
  P02 passes strictly iff gap + M - dAW < 0, M = d1+d2 (our pts minus his); tie iff M == dAW - gap.
  Tie counted AS LOSS in headline numbers (conservative; in truth we hold the pleno tiebreak 18 vs
  his ~14-16) -- both printed.
ASSUMED inputs (labeled, swept): P02 side-probabilities Q3_ENG / QF_ARG (the free variables -- QF_ARG
  ...
  Simon presumptive if ESP; assists: Olise 5 vs Messi 4). Cross-validated by p02_endgame_mc.py
  (independent vectorized MC, seed 42) within 3xSE -- the L62 rule.
Usage: python council/outputs/endgame/p02_endgame.py [snapshot.json]
"""
import sys, os
...
# P02 entry model (ASSUMED shapes, SF2-continuity; side-split = the swept free variable)
G3_FRA = {(2, 1): 0.35, (1, 0): 0.35, (2, 0): 0.30}
G3_ENG = {(1, 2): 0.40, (0, 1): 0.35, (0, 2): 0.25}
Q3_ENG = 0.30            # P(P02 goes England-side in the 3rd-place game); no stake -> mild contrarian
GF_ESP = {(1, 0): 0.40, (2, 1): 0.35, (2, 0): 0.25}
GF_ARG = {(0, 1): 0.35, (1, 2): 0.35, (0, 2): 0.15, (1, 1): 0.10, (2, 2): 0.05}
QF_ARG = 0.50            # P(P02 goes Argentina-side in the FINAL) -- THE read; full 0..1 sweep below
...
def diff_dist(cells, entry, gmix):
    """Exact dist of d = pts(entry,cell) - pts(g,cell) over cells x P02 mix."""
...
def award_paths(champ):
    """Exact (label, prob, dAW) enumeration for one realized champion. dAW = P02-minus-us."""
...
def main():
    ...
    print(f"P02 ENDGAME (exact, no MC)  snap={os.path.basename(SNAP)}  gap +{GAP}")
    ...
    print(f"  P02 model: Q3_ENG={Q3_ENG}  QF_ARG={QF_ARG} (ASSUMED, swept)  awards: yamal {YAMAL}, "
          f"messi {MESSI}, dibu {DIBU}, kane {KANE}, maignan {MAIGNAN}, pens_esp {PENS_ESP}")

    # STRUCTURAL CHECK: no award delta -> P02 cannot pass (max match swing 18 < 24)
    ...
    print(f"  STRUCTURAL: worst joint match swing = {worst} > -{GAP} -> awards are MANDATORY for P02  OK")

    # ---------- [A] e2 (FINAL) decision table at gap +24, vs QF_ARG sweep -> crossover ----------
    print("\n=== [A] FINAL entry e2 at gap +24 (game1 net 0): P(P02 passes+ties) x QF_ARG ===")
    ...
    # ---------- [C] P02-NEEDS: our chalk (2-1, 1-0) vs his loss paths, named ----------
    print("\n=== [C] P02-NEEDS MAP (our entries = chalk 2-1 then 1-0; P02 mix at defaults) ===")
    d1 = diff_dist(cells3, (2, 1), g3)
    reg10 = regs[(1, 0)]
    tp, tt = joint(d1, reg10)
    print(f"  P(P02 passes)={tp:.6f}  P(exact tie)={tt:.6f}  [P(hold strict)={1-tp-tt:.5f}]")
    ...
    # ---------- [D] ADVERSARIAL: P02 plays a PURE best-response (he can't see our entry) ----------
    print("\n=== [D] ADVERSARIAL P02 (pure entries, max over his (g1,g2)): our candidate pairs ===")
    gset3 = list(G3_FRA) + list(G3_ENG) + [(3, 1), (1, 3), (3, 0), (0, 3), (2, 2)]
    gsetF = list(GF_ESP) + list(GF_ARG) + [(3, 1), (1, 3), (0, 0)]   # long-shot gambles included
    print("  ours(e1,e2)      worst-case P(pass+tie)   P02's best response (g1,g2)")
    results = {}
    for e1 in ((2, 1), (1, 0)):
    ...
    print("\n=== [E] FINAL-node lookup: best e2 by realized gap (after 3rd place; P02 mix default) ===")
    ...
    print("\nHAND-CHECKS PASS. All probabilities exact (convolution); MC cross-val = p02_endgame_mc.py.")
