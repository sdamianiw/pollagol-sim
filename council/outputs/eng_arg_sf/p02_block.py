# -*- coding: utf-8 -*-
"""P02-BLOCK decision harness (2026-07-15, ADVISORY; I-3 clean, imported by nothing).
THE architecture call of SF2: gap vs P02 collapsed to +24 and Sebas holds a ~95% prior that P02
...
unimplementable), so the decision metric tonight is argmax_e P(hold #1 vs P02 | our entry e),
computed jointly with the post-SF1 endgame (Spain 2-0 France -> ESP in final; FRA legs DEAD).
Chain per MC draw (seed 42): tonight's real 120' cell (frozen FULL120 dist) x P02 entry ~ G
...
# P02 entry model G (ASSUMED shape, swept): P(ARG-side) = p95; within-side weights below.
...
AW = {"yamal_mvp_if_esp": 0.25, "yamal_mvp_else": 0.05,
      ...
      "mbappe_boot": 0.40}       # common-mode vs P02 (both hold it) -- kept for completeness
...
def run(d, cells, ps, p95, g_arg, yamal, messi_fin, pair_esp, label, verbose=True):
    ...
    # 2026-07-15 by the exact-enumeration cross-validation in p02_paths.py)
...
def main():
    ...
    print(f"  P(pass) ~ Phi(-{z:.2f}) ~ {0.5*(1-erf(z/sqrt(2))):.5f} per chaser -> negligible; P02 is the node.")
