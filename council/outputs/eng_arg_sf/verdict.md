...
EXACT loss-path enumeration (`p02_paths.py`, pure convolution, no MC) as a cross-validation of the
`p02_block.py` MC — and they DISAGREED 15×. Root cause: **numpy boolean `+` is logical OR**, so
`LOCK * ((champ=="ESP") + (mvp=="Yamal") + ...)` capped every award payout at ONE leg — **P02's
...
- **1-0 England: P(hold #1) = 0.9918** (P(P02 passes/ties) = 0.82%)
- **0-1 Argentina: P(hold #1) = 0.9961** (0.39%) → **the cover HALVES the loss tail, buying ~42bp**
  ...
  precisely that branch (tonight contributes ~0 instead of −5..−9, forcing P02 to find the swing from
...
**FINAL: ENTER ARGENTINA 0-1** (P02-cover; his modal pick). T-15: if fresh ARG devig ≥ .34 the ENGINE
also flips to 0-1 and the entry is unanimous; no fresh number reverses the cover while P02 is ARG-side.
...
P02-BLOCKING node — a NOVEL decision type: EV-argmax vs P(hold #1) cover of a ~known chaser.**
...
## The P02-block harness (`p02_block.py`, CONTROL identity PASS ×3, B4, floor-check others 0.0003)
- **0-1 Argentina MAXIMIZES P(hold #1 vs P02) = 0.99984** vs **1-0 England (EV-argmax) = 0.99945** →
  **+0.00039 (4bp)**. STABLE: 0-1 wins P(hold) in ALL 9 sweep cells (p95 .80–1.00, yamal .15–.40, messi
  .05–.25, pairESP .40–.60). Mechanism: 1-0 ENG is ANTI-correlated with P02 (Argentina win → he scores,
...
| Adversarial | **England 1-0** | High | 4bp is SUB-NOISE (inside ±1-2pp P02-prior error, a subjective read); concave prize-utility flat near P=1 (~$0.02/bp); 0.12 EV + pleno cost is real & certain; backfire tail if P02→England |
...
1. **Fragile signal vs robust cost.** The 4bp P(hold) edge rests on a *subjective* 95% P02-prior + ASSUMED
...
2. **The cover defends the wrong flank.** Tonight's scoreline choice only touches MATCH variance (±9, vs a
   +24 lead). The real residual threat to #1 is P02's **Yamal-MVP + Messi-assist award parlay in a
...
4. **Preserve the tool.** Covering value is highest when the race is TIGHT and games are FEW; tonight
   (+24, 3 games) is its lowest-value moment. Spend it at the FINAL if P02 is within ~10 and his pick is
   known.

## Bias statement (per mandate)
Sebas leaned "pick the same as P02 — can't afford the risk." The council VINDICATES the instinct's
...
disciplined entry (England 1-0) is what a neutral +24 leader enters; it survives removal of the P02-fear.
...
- **Covering doctrine (NEW, logged):** re-arm the cover at the FINAL if gap ≤ ~10 and P02's pick is known.

## FINAL WORD (delivered ~17:40Z, T-1h10): **ENTER ENGLAND 1-0.** Cover DECLINED (4bp, sub-material,
model-fragile; the EV+pleno edge is robust; the real threat is P02's award parlay, not tonight's score).
