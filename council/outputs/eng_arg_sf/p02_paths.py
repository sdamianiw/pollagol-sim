# -*- coding: utf-8 -*-
"""P02 LOSS-PATH EXHAUSTIVE ENUMERATION -- 'every universe where P02 wins' (2026-07-15, ADVISORY;
I-3 clean, imported by nothing). EXACT computation, no Monte Carlo: full convolution over
  tonight's 120' cell (frozen FULL120 dist) x P02's entry (mix G) -> conditional d1 | result-region
  x 2 proxy games (exact convolution) x champion chain | region x award categoricals | champion
  -> P(P02 passes) decomposed into NAMED PATHS, for our entry e in {1-0 ENG, 0-1 ARG, 2-1}.
CROSS-VALIDATION (meta-check): totals must reproduce p02_block.py's MC loss rates (1-P(hold):
...
Same ASSUMED inputs as p02_block.py (P02 mix p95=.95, award conditionals, PAIR_ESP .50), labeled."""
import sys, os
...
def award_paths(region):
    ...
    Returns list of (label, prob, dAward = P02-minus-us award delta). Common legs excluded."""
...
def analyze(entry, cells, gmix, proxy2, gap, verbose=True):
    ...
    for r in ("ENG", "ARG", "LEV"):
        ...
        for label, pa, daw in award_paths(r):
            # P02 passes strictly if gap + M - daw < 0 -> M <= daw - gap - 1 (integer support)
            thr = daw - gap - 1
    ...
    if verbose:
        print(f"\nENTRY {entry[0]}-{entry[1]} @ gap +{gap}:  P(P02 PASSES)={tot:.6f}"
...
def main():
    ...
    print("\n=== CROSS-VALIDATION: exact (this harness) vs FIXED p02_block MC N=200k seed42 (post-L62) ===")
    ...
    print("\n=== GAP SWEEP -- 'when to play the man' (P(P02 passes+ties), EV-pick 1-0 vs cover 0-1) ===")
