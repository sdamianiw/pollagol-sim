...
AWARD = {
    ...
    "messi_ast_any":      0.05,  # P02 (Messi PROBABLE 3 vs Olise 5)
    "mbappe_ast_if_fra":  0.10,  # Rodrigo (Olise leads by 2 on his own teammate)
    "mbappe_ast_else":    0.02,
}
# our Mbappe-BOOT differential vs P05/Rodrigo (they don't hold Mbappe scorer): 8==8 tie with Messi
...
def main():
    ...
    for tag, v in (("mbappe_boot_lo", -0.10), ("mbappe_boot_hi", +0.10)):
        ...
        d_fel = locked50_delta("P05", "FRA")
        MBAPPE_BOOT.update(old)
        print(f"  {tag}: P05-delta(FRA-branch)={d_fel:+.2f}")
    for dp in (-0.10, +0.10):
