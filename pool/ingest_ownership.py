...
def blanks_chalk_resolved(named_shares: dict, blank_mass: float, p_true: dict) -> dict:
    """Model B (P02-a): redistribute the TRUE-blank mass over the NAMED candidates' P_true, renormalized
...
def load_observed_from_snapshot(path: str, board_keys=None) -> dict:
    ...
    board_keys: {lever: set(canonical board names)} for off-board detection (P02-b). A name that resolves
        ...
        if advisory or lever not in board_keys:
            ...
            for team, c in counts.items():
                ...
                if team in keys:
                    ...
                    offboard[team] = c
                    print(f"  [P02-b] observed pick {team!r} ({lever}) without P_true market -> "
...
def _selftest():
    ...
    # Model B (P02-a): redistribute the blank mass over named P_true (renormalized over named) -> still sums to 1.
