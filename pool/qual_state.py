...
SIGNAL TAXONOMY:
  (A) Per-OUTCOME classifier - exhaustive (s,t), mutually exclusive (no implicit else, no UNCLASSIFIED).
      For team T over its 3 group-mates: s = #others with pts > pT ; t = #others with pts == pT.
        ...
        s + t <= 1        -> TOP02-GUARANTEED  (top-2 regardless of any GD/H2H tiebreak)
        else (s<=1, s+t>=2) -> TOP02-TIEBREAK  (2nd-vs-3rd hinges on tiebreak -> flagged, NOT resolved;
  ...
  (B) Per-TEAM state - from T's own-result partition (conservative: a tie for 2nd is NOT a guarantee).
      win_g/draw_g/loss_g = TOP02-GUARANTEED in ALL outcomes where T wins / draws / loses.
...
def _classify(p_t: int, others: list[int]) -> str:
    """Exhaustive (s,t) per-outcome classifier. Returns ELIMINATED | TOP02-GUARANTEED | TOP02-TIEBREAK."""
    s = sum(1 for p in others if p > p_t)
    t = sum(1 for p in others if p == p_t)
    if s >= 2:
        return "ELIMINATED"
    if s + t <= 1:
        return "TOP02-GUARANTEED"
    return "TOP02-TIEBREAK"                                  # s<=1 and s+t>=2 (exhaustive remainder)
...
def qualification_states(table: dict, game1: tuple, game2: tuple) -> dict[str, dict]:
    ...
    for tm in teams:
        ...
        for (r1, r2, fp, cls) in outcomes:
            ...
            if cls[tm] == "TOP02-TIEBREAK":
                tb_count += 1

        def all_guar(lst):
            return all(c == "TOP02-GUARANTEED" for c in lst)
        ...
        def relaxed(lst):                                  # treat TIEBREAK as if GUARANTEED
            return all(c in ("TOP02-GUARANTEED", "TOP02-TIEBREAK") for c in lst)
...
def _selftest():
    ...
    assert _classify(5, [5, 5, 3]) == "TOP02-TIEBREAK"          # (s=0,t=2)
    assert _classify(5, [5, 5, 5]) == "TOP02-TIEBREAK"          # (s=0,t=3)
    assert _classify(5, [7, 5, 5]) == "TOP02-TIEBREAK"          # (s=1,t=2)
    seen = set()
    for s in range(4):
        for t in range(4 - s):
            others = [10] * s + [5] * t + [0] * (3 - s - t)
            seen.add(_classify(5, others))
    assert seen <= {"ELIMINATED", "TOP02-GUARANTEED", "TOP02-TIEBREAK"}
