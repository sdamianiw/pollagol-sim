...
> (Track B P0–P4 below), the ~88%-of-points lever (L6), ahead of DoD-3. **P0–P02 DONE** (engine M1–M6,
...
- [x] Verdict in `memory/rules.md` ("P1 verdict — M7 historical-odds source") = source + mode + leakage
      rule (`odds_timestamp < kickoff`) + transfer caveat. **STOP for P02 GO.**

### P02 — Build M1–M6 — DONE 2026-06-03 (TDD RED→GREEN; 45/45 suite GREEN, no regression; e2e smoke OK)
...
### P02 — build `pool/podium_montecarlo.py` (joint K-lever) — DONE 2026-06-05
...
- [x] **PODIUM TEST RUN — observed-ownership rehearsal — DONE 2026-06-06 (suite 137/137):** `[OBS]` block in
      ...
      `blanks_chalk_resolved`, `load_observed_from_snapshot` (P02-b off-board→fold+log) +
