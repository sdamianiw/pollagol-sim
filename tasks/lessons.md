...
AGP02ATE points/match vs B1 — but the engine provably EQUALS B1 on clear favorites (both pick fav-1-0,
...
## L12 — The contrarian edge is σ- and lever-count-fragile; K=1 intuition does NOT transfer to K>1 (caught 2026-06-05, P02; demonstrated)
...
despite two favorites dying we stayed RANK 1 at 264 (+4 over P17 260; gaps +4/+9/+14 PRESERVED) because the draws
...
- 2026-07-01 · NEW `src/ko_cadence.py` + `tests/test_ko_cadence.py` (17) + `.claude/skills/ko-matchday-cadence/SKILL.md` (KO cadence skill — formalizes the scratchpad flip-check + ko_adjust drivers into a deterministic tested CLI; non-frozen, reuses FROZEN engine + ko_adjust verbatim, imports only PUBLIC symbols, I-3 clean, golden values pinned to the Belgium-Senegal run) · `/code-review` **high (8 angles / 4 finder agents)** → **10 findings FIXED** (3× None-deref on ko_adjust `best_draw=None` [cand_set / f-band / _print_ko], empty-table IndexError in fixture_eval, LineGuardStop batch-abort → GUARD-STOP verdict, I-3 test CWD-relative + inside skipUnless → moved to an UNCONDITIONAL class + `inspect.getsource`, private `_select_event` import from frozen run_matchday → local reimpl, `ko_rule` un-threaded → `--ko-rule`, ko_flip buried on the council line → promoted to the FULL120 PICK line, self-baseline double-load → fresh-scan refactor) + **2 skipped with reasoning** (`_score`/`_parse_score` dups — both codebase formatters are PRIVATE so nothing public to reuse, and importing the frozen `decision_score` scoring module into a read-only cadence tool for a 2-line parser is worse coupling; ko_adjust 0.65 recompute — negligible on a read-only CLI, a float-compare guard is more fragile than the ~1ms save) · suite **251→268** green, frozen diff EMPTY, E2E reproduces the manual P02/P3 tables. Commit `cda4ddb`. **Lesson: a durable reusable tool earns robustness a one-session scratchpad skips** — the review's top 5 bugs (None-derefs, empty-table, batch-abort) were all "impossible in tonight's data" but reachable on future KO boards; formalizing scratchpad→module is exactly when to pay that down.
...
- 2026-07-12 · QF close-out reconcile (377, pleno #17) + SF prep session (fetch/flipcheck/ko both SFs + 3 harnesses + sourced award refresh) = DATA-OPS + 3 ADVISORY harnesses committed as council artifacts (`sf_prep/{drift_fra_esp,drift_eng_arg,lead_max_sf,endgame_branches_sf}.py` — imported by nothing) → `/code-review` on the staged diff (advisory scope; 2 finders): **4/4 findings CONFIRMED+adopted — (a) grid-label sign inversion in BOTH drift files (template's focal team was AWAY, ours are HOME; labels said FRA−4pp while computing FRA+4pp — boundaries/stability unaffected because the grid is symmetric, but the per-row reading was inverted; NEW RULE: when adapting a drift harness, re-derive the focal-team slot, don't copy the ± dict), (b) dibu-sweep printed insensitive cells (now prints the chaser×branch where the key fires), (c) verdict +33→+34 rounding, (d) 'ENG/ARG mildly favorable' now names the sole positive cell (P11 +1.9/+0.5)**; all fixes re-run B4; RED gates = frozen-diff EMPTY start+end · board arithmetic settled a HUMAN-RECALL conflict (Sebas said ARG "won 1-0"; screenshot notation pick(actual) 1(3)-0(1) + board Σ 365+9+3==377 proved 3-1 → user confirmed; **the board-total cross-foot is the standing arbiter of result reports — trust it over memory, mine AND his**) · L61 replayed by DESIGN this time (pop/re-append the newer row because `backfill` aborts on absent-fixture blanks — the one-fixture-at-a-time rule is a tooling constraint, not a preference) · fetch 2/2==calendar quota 460 (SF2 ENG home vs Argentina bound by full string) · flipcheck HOLD + NO-BASELINE, B4 everywhere · CONTROL==LIVE ≤6.7e-15 ×2 drifts + committed-QF-endgame reproduces its committed artifact + lead-max CONTROL==+0.1084 hard-assert · 3 rubric cells + 2 branch rows hand-checked == points()/harness · I-3 clean. NEW patterns: (1) **razor-on-the-boundary protocol** — when the argmax crossover (SF1 p_over .49) sits INSIDE the observed inter-day oscillation (.480↔.500), the deliverable is the boundary + a T-1h decision rule, NOT a lean; (2) **winner-boundary beats EV-gap as the tightness metric** — ENG-ARG looks decisive by EV (+0.18) but flips winner at ARG +2pp, the tightest node yet; (3) **award-narrative shocks belong in the endgame dict, not the match layer** — Bellingham's brace cut kane_mvp_if_eng .55→.30 and changed NO hold-sign (sweeps prove pride ≠ risk).
- 2026-07-14 · SF1 council day (T-2h fresh fetch + fresh-odds verdicts BOTH SFs + variable-change adjudication + 4-lens PARALLEL council) = DATA-OPS + a 4-line argv-override code diff on the sf_prep harnesses (advisory, non-frozen) + new verdicts `council/outputs/fra_esp_sf/verdict.md` + `eng_arg_sf/verdict_preliminary.md`. RED gates = fetch 2/2==calendar quota 458 · flipcheck 2/2 HOLD + B4 · fresh ko both SFs · argv edit CONTROL-proven INERT on old inputs (all 4 reproduce committed *_run_2026-07-12.txt byte-identical) + fresh B4 · lead_max CONTROL +0.1084 · frozen diff EMPTY · I-3 clean. **Divergence scan vs Jul-12: BOTH favourites softened toward their underdogs (FRA −1.2pp/ENG −0.7pp; ESP/ARG +1.1pp each) = one coherent common-mode pattern, SUB-THRESHOLD on every boundary — zero pick changes, P(hold #1)≥.998 unchanged.** Verdicts: SF1 = FRANCE 1-0 (p_over .4797<.49; crossover held at .49; the LIVE axis is Spain firming toward the A≥.34 winner-flip via the France CB question, NOT totals). SF2 = ENG 1-0 preliminary (winner-flip A≥.34, ARG +2.3pp away & trending). **KEY PROCESS LESSON (Sebas caught it): I wrote the FRA-ESP council lenses INLINE instead of spawning isolated parallel subagents — a self-synthesized "council" launders my own prior into all 4 voices and defeats the isolation that catches blind spots. FIX: launched 4 blind parallel lens agents (form/market/leader-bias/adversarial); reconciled as final judge. The fan-out EARNED its cost — 3 independent lenses triangulated onto the SAME hinge (sharp handle on Spain + CB scratch + tactical crack = winner-side is the live axis) that my inline version had buried as reopen-trigger #3, and the bias-guard got an independent HIGH-confidence "pure tilt" ruling. RULE: a MANDATORY deep council (esp. champion/bias node) MUST be isolated parallel subagents + judge reconciliation, never inline self-lensing — the whole value is independence from the synthesizer's prior.** Also caught+ran-down an incongruence: a news source claimed "Argentina favored" for SF2; raw Pinnacle price (ENG 1.75 < ARG 2.05) + 25-book devig (ENG .366>ARG .317) REFUTED it — England is the book favourite; the engine's fetched odds are the arbiter (I-3). /evaluation (SKILL 5.md) rubric applied to the plan pre-approval = 10/10 PASS (Layer-1 principles); no new gate built → no Layer-2.
- 2026-07-15 · SF2 P02-BLOCKING node (gap +24, P02's ARG pick ~95% known) — THE EVALUATION LOOP FLIPPED THE VERDICT. **L62 (CRITICAL, numpy): `bool_array + bool_array` is LOGICAL OR, not addition — `LOCK*((champ=="ESP")+(mvp=="Yamal")+...)` capped every locked-50 payout at ONE leg, so multi-leg award PARLAYS never stacked in p02_block.py (and p_first_multiverse.py, annotated). The buggy MC said the P02-cover buys 4bp ("sub-noise, decline"); the goal-mandated /evaluate built an EXACT convolution enumeration (p02_paths.py) as cross-validation → 15× disagreement → root-caused → fixed (.astype(float) per leg) → two independent implementations now agree (0.824% vs 0.826%).** CORRECTED: cover buys ~42bp (P(P02 passes) HALVES 0.82%→0.39%), sign-robust ×9 sweeps, grows in adverse cells (58-77bp); top loss path under 1-0 = ARG-win×Yamal×Messi parlay needing only M≤−5 = 42% of losing universes — exactly what the cover closes. **VERDICT CORRECTED: ENTER ARGENTINA 0-1 (play the man).** 3 council lenses had ruled HOLD on the false 4bp premise; bias-hygiene lens (P(1st) governs, cover=argmax, Sebas's instinct vindicated) stands. RULES: (1) any harness summing boolean indicator arrays MUST cast per leg — grep for `== "` inside arithmetic; (2) an MC with a decision on the line gets an EXACT cross-validation when the state space allows it (convolution beat 200k draws AND found the bug the CONTROL identity couldn't — the identity checked match-points, not award arithmetic); (3) meta-check gates must cover EVERY payoff component (the award path had no hand-check — now it does); (4) L50-exception precedent: a ~known single-chaser entry makes defensive-chalk implementable, and at 40bp+ it is ACTIONABLE (gap-sweep doctrine: +24→42bp cover, +20→80, +15→163 — cover earlier than intuition says).
- 2026-07-15 (FINAL GATE) · SF2 harness hardening + /evaluate QC. Ran p02_paths.py ×3+committed = 1 hash (idempotent/deterministic). Two PARALLEL isolated bug-hunt agents (adversarial-arithmetic + metacognitive-cross-harness) — both fully corroborated the independent audit, ZERO new findings beyond the 2 I'd already caught+fixed: (a) LEV-region Messi-assist over-averaged (flat .10 ignored that champion is correlated with Argentina-reached-final when tonight goes to pens → champ-aware pm .15/.05/.10; impact ~1.1e-5, immaterial); (b) stale cross-validation reference (p02_paths compared exact vs the PRE-L62 buggy MC constants 0.00055/0.00016 → updated to fixed 0.00824/0.00388 with a principled 3×MC-SE tolerance; now PASSES). Divergence exact 0.00414 vs MC 0.00388 on 0-1 = PURE MC NOISE (proved: N=1M MC → 0.00417, converges onto exact; all 3 entries within 2 SE). **L62-CLASS SWEEP (complete): bug isolated to p02_block.py (FIXED) + p_first_multiverse.py lines 139-144 AND 202-208 (annotated, superseded — NOT a live decision input); endgame_branches_sf/worst_world_stress/lead_max_sf/p02_paths all CLEAN (scalar-E[] or exact-enumeration, immune by construction).** **KEY META-LESSON (reward-hacking / gate coverage): p02_block's CONTROL identity (E[diff]==dEV) PASSED THROUGHOUT — it only checked the MATCH-points arithmetic, not the AWARD-settlement arithmetic where L62 lived. A CONTROL that covers only part of the computation gives false confidence; the bug was caught ONLY by an INDEPENDENT exact-enumeration cross-validation (p02_paths convolution) disagreeing. RULE: for a decision-critical MC, the meta-check must be an independent re-derivation of the WHOLE output (ideally exact when the support is finite), not a partial internal identity.** VERDICT UNCHANGED + hardened: ENTER ARGENTINA 0-1 (cover buys ~41bp at +24, sign-stable ×9 sweeps, 2:1 loss-ratio robust to the residual noise). pytest 269/269, frozen 0, I-3 clean. NO code do-over of the analysis — the flip (Eng→Arg) was the evaluation catching a bug, and this gate only hardened the already-correct call.
- 2026-07-17 · ENDGAME council (3rd-place + FINAL baselines) — **L63: a cover doctrine is a TRIGGERED rule, not a momentum play; re-derive its trigger at every node.** SF2's cover (42bp) existed because 3 games remained (27 max match swing > gap 24 = live mirrorable threat). With 2 games (18 < 24) the match channel is EXTINCT, award legs are score-independent, and the exact harness (p02_endgame.py, MC cross-val 5/5, joint q×award sweeps) shows the cover buys ≤2-3bp at ANY q under the strict-pass metric (we win ties, plenos 18) → **chalk-chalk FRA 2-1 + ESP 1-0, unanimous 4 isolated lenses**; the tight "read P02" question proved q-INSENSITIVE. Corollary rules: (1) decision metric = STRICT pass when we hold the tiebreak (tie-as-loss overstated risk 60%); (2) sweep the DECISION variable jointly with parameter stresses (adversarial lens caught [F] sweeping awards only at q=.5 — the crossover q*=.81 lived outside it); (3) a P02 ROLE-PLAY agent independently derived the same best-responses as the exact minimax ([D] g2=2-1 ESP; his only viable universe = Spain title, ~1% self-assessed) — role-play is a cheap corroboration channel for adversarial models. Gates: SF2 recorded (1-2 double pleno, Σ==386==board, override +30 reconciled, 18 plenos), fetch 2/2==calendar, B4 ×3, pytest 269/269, frozen 0.
- 2026-07-17 (B6 two-front) · P02's seed-2-vs-#1 dilemma resolved numerically (p02_two_front.py, MC 1M seed 42, REAL ownership slates, 2-agent metacognitive+adversarial panel + role-play, all SURVIVES; V1 caught an inverted sweep sign -> fixed; V2 exact-derived the S5 hedge + the new structural fact: **P02's Espana champion lock makes his seed-2 unassailable in ALL Spain-title worlds (28<30)**). His E[prize] is FLAT (23bp of pot = 1.9k CLP): England-gamble-alone = his worst move (P3 x2.6), fandom-ARG-alone weak, all rational profiles Spain-side-or-noise. RECONCILED vs 54afad2: every P02 model lands inside the [D] adversarial ceiling -> **CHALK-CHALK RE-CONFIRMED, third independent confirmation**. Rule: when modeling a rival, model BOTH his fronts — a chaser with a prize-graded seat below him is not a pure P(#1)-maximizer; E[prize] flattens his strategy space and usually pulls him toward chalk.
- [2026-07-19 FINAL-node gate-log] Record FRA-ENG 4-6 (Sigma==386 PASS) · last fetch quota 444 1/1==calendar · flipcheck HOLD +0.0000 x2 baselines B4 · ko FULL120 1-0 ESP E 2.713, triggers quiet (p_over .417, ARG .274) · final_node.py driver (reuses p02_endgame fns, B4 1-hash): at realized gap +24 the 0-1 cover is NEGATIVE-value in every stressed sweep (-14 to -54bp) and 1-0 ESP = EV-argmax AND P(hold)-argmin for q<=.75; Olise +1 assist (7, record) kills Messi-AST -> P(hold strict) 99.976%. Pattern: a resolved game-1 collapses a joint harness to its [E] lookup — drive it with a thin reuse driver, never re-derive logic.

## Gate-log — 2026-07-21 · TOURNAMENT CLOSE-OUT (final record + standings + wrap)
DATA-OPS, no code diff → /code-review N/A. RED gates: final result ×5 sources (Spain 1-0 Argentina, Torres
106' ET → FULL120 "1-0") · record → n=104, Σ us_entered=395, 19 plenos, override +30 (final row +0, pick==entered==actual PLENO) ·
board cross-foot 395 + 15 premiaciones == 410 ✓ · standings ingest 2026-07-21 (rank 1/27, z 2.73) · frozen
diff EMPTY · I-3 clean. **New datum/pattern: the FINAL board mixes match + premiaciones — the cross-foot
identity extends to Σ_CSV + awards_settled == board. Awards observed +15 (champion ✓ + Boot ✓) vs
...
< P02 386 → model alone loses by 1; the gated-override layer (+30 deliberate) is what won. L64 logged.
