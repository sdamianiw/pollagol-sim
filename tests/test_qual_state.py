...
class TestClassifier(unittest.TestCase):
    def test_6a_multi_tie_outcomes(self):
        self.assertEqual(_classify(5, [5, 5, 3]), "TOP02-TIEBREAK")   # s=0,t=2 (triple-tie-1st)
        self.assertEqual(_classify(5, [5, 5, 5]), "TOP02-TIEBREAK")   # s=0,t=3 (quad-level)
        self.assertEqual(_classify(5, [7, 5, 5]), "TOP02-TIEBREAK")   # s=1,t=2 (one-above-triple-tie)
        self.assertEqual(_classify(5, [3, 3, 3]), "TOP02-GUARANTEED")  # s=0,t=0
        self.assertEqual(_classify(5, [7, 3, 3]), "TOP02-GUARANTEED")  # s=1,t=0
    ...
    def test_6b_exhaustive_sweep_no_unclassified(self):
        valid = {"ELIMINATED", "TOP02-GUARANTEED", "TOP02-TIEBREAK"}
        for s in range(4):
            for t in range(4 - s):
